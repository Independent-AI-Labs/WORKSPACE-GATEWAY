...
| FR-2.4 | The migrator MUST resolve billed cost for **every** row provider-scoped from the provider's `pricing.overrides` first, else models.dev (per-1M rates `input`/`output`/`cache_read`/`cache_write`/`reasoning`, formula mirroring `plugins/custom/cost_calc.lua`: uncached input at `input`, non-reasoning output at `output`, cached at `cache_read`, cache write at `cache_write`, reasoning at `reasoning`), marking `cost_source='provider_override'` or `cost_source='models_dev'` accordingly. The opencode-recorded message cost MUST be persisted to `reported_cost` as metadata and MUST NOT be billed nor set `cost_source`. Provider resolution MUST be explicit and single-step: apply `cost_calc.PROVIDER_ALIASES`; if the result is a gateway provider (in `conf/providers/*.yaml`) the models.dev namespace is its declared `pricing.source.provider` (empty when the type is not `models_dev`), otherwise an explicit direct-provider equivalence (`zai-coding-plan` → `zai`) applies, and failing that the historical providerID is the models.dev namespace itself. The lookup key is `models_dev_namespace : canonical(modelID)`. A rate models.dev omits MUST be applied as the live path does: reasoning bills at the output rate when no reasoning rate is published, and cache_read/cache_write bill at the input rate when omitted or zero (never free). A miss stays unpriced (no provider-agnostic rule), keeping cost 0 with `cost_source='unknown'`. |
...
| ID | Requirement |
|----|-------------|
| FR-4.1 | `--dry-run` MUST report, without writing: per-table row counts to insert, source duplicates collapsed, ClickHouse rows already present, rows older than 13 months (reporting bucket; retention is indefinite per REQ-SECURITY-HARDENING FR-4), pricing coverage (rows with cost = 0, how many resolve via the catalog, how many stay unknown), and a source-vs-target diff (missing / differing / extra `event_id` counts, with up to five sample differing rows). |
...
| FR-4.7 | The pricing catalog MUST come from models.dev (`MODELS_DEV_URL`, default `https://models.dev/api.json`) or an explicit `--pricing-file <path>`; a missing `--pricing-file` path or an unreachable URL MUST fail the run rather than continue with every row unknown. Provider resolution to the models.dev namespace follows FR-2.4. |
