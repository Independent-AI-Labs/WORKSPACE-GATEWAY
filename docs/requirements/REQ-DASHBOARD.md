...
| ID | Requirement |
|----|-------------|
| FR-1.1 | The system SHALL provide exactly 5 dashboards: `gateway-cost-usage` (4 CH panels: ids 3, 15, 8, 46), `gateway-ops-health` (11 panels: ids 1, 2, 4, 5, 7, 10, 11, 12, 14, 50, 51  -  7 CH + 4 Prom; p50/p51 storage footprint per REQ-SECURITY-HARDENING FR-4.3), `gateway-cost-leaderboard` (4 CH stat panels: ids 20, 21 podium + 22, 23 runner-ups), `gateway-model-experience` (6 CH panels: ids 34, 37, 40, 42, 43, 47; satisfaction signals, Usefulness Score + score cards, friction: behavioral requirements owned by REQ-USEFULNESS-TELEMETRY), and `gateway-model-performance` (6 CH panels: ids 30, 31, 36, 44, 45, 48; speed, cancel/abort, waste, TTFT: owned by REQ-USEFULNESS-TELEMETRY). |
...
| FR-2.2 | Active connections, request rate, bandwidth, and shared-dict memory MUST use Prometheus for real-time `rate()` semantics. |
...
| FR-4.4 | p4 (Error Rate) MUST compute `countIf(status >= 400) * 100 / count()` (all 4xx + 5xx), with a single coral threshold band so the tile background is red at any measured rate (errors are never a "good" green). |
...
| FR-4.8 | RETIRED (2026-09-22): p13 (Stream Abort Rate) was removed as operator surplus - it was the same abort split as p14 expressed as a percentage; p14 covers it. |
| FR-4.9 | p14 (Stream Status) MUST show stacked absolute counts for completed/client-aborted/provider-aborted streams. |
| FR-4.10 | RETIRED (2026-09-22): p9 (Latency Percentiles) was removed as operator surplus - it plotted only APISIX's own sub-5 ms processing overhead, not a useful health signal; p10 covers per-model response time. |
...
| FR-4.17 | p50 (Storage Footprint: hot vs archive) MUST show active bytes-on-disk per storage tier (`if(disk_name = 'archive', 'archive', 'hot')` over `system.parts`); p51 (Storage Footprint by Table) MUST show active bytes-on-disk per `llm_gateway` table (top 10). Both live on `grafana_ro`-granted system tables only; a unified alert MUST fire when free space drops below 20% (REQ-SECURITY-HARDENING FR-4.3). |
...
|------|--------|----------|
| FR-1.1 5 dashboards / 31 panels | Implemented | conf/grafana/dashboards/*.json (4+11+4+6+6 panels; experience: score/score-cards/rejection/friction, performance: prefill/decode speeds split p30/p44 + reliability + TTFT + waste + completed-response averages; ops-health p50/p51 storage footprint lands with REQ-SECURITY-HARDENING) |
| FR-1.2 time range & refresh | Implemented | each dashboard: `now-90d`→`now`, `5s` |
| FR-2.x datasource split | Implemented | 27 CH + 4 Prom panels across dashboards |
