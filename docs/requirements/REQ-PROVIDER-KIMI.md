...
> the `provider-oauth` plugin, token custody in OpenBao with transparent refresh, and
...
- [`plugins/custom/provider-oauth.lua`](../../plugins/custom/provider-oauth.lua): device start/poll/proxy phases
...
- `provider-oauth` proxy authentication, session lookup, and automatic refresh
...
| FR-1.3 | `/kimi/*` routes MUST use `provider-oauth` (not `key-resolver`); `/kimi-federated/*` routes MUST use `key-resolver` with `upstream_key_env: KIMI_API_KEY` and `virtual_key_prefix: vgw-`; `/kimi-key/*` routes MUST use no auth plugin. |
...
| FR-1.5 | The device endpoints `/kimi/auth/device` and `/kimi/auth/device/poll` MUST be handled inside `provider-oauth` before any upstream proxying. |
...
| FR-5.2 | All three MUST source models from models.dev namespace `moonshotai` with `strip_prefix: moonshotai/` + `lowercase` normalization and `pricing.source.type: models_dev`, `pricing.source.provider: moonshotai`. |
...
| C-2 | `provider-oauth` priority 2560 (before key-meta 2530) | plugins/custom/provider-oauth.lua |
...
| FR-2.x device flow | Implemented | provider-oauth.lua, oauth_device.lua |
| FR-3.x proxy auth + refresh | Implemented | provider-oauth.lua `plugin.access` |
