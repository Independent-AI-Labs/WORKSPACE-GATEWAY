...
| FR-10.5 | Panels are split across two dashboards, each grouped in tiers. `gateway-model-experience`: Verdict (score + scorecard) → headline stats (rejection, baseline-net, session depth) → top strings + friction + guard rules → model switch. `gateway-model-performance`: prefill p30 / decode p44 side by side (separate panels so the ~40k prefill and ~100 decode scales never share an axis) → cancel/abort + waste + completed-response averages. Row-keyed bargauges use all-values reduce (one gauge per model). |
| FR-10.6 | The scorecard table MUST merge each rate column with its normalized index into a single cell (`"1.48% / 0.70"`) so headers fit without truncation, and Overall Score + scorecard MUST honour an `include_local` toggle (default exclude) filtering `llm_gateway.model_registry WHERE is_local = 1`, materialized from `conf/providers/*.yaml` `local: true` flags by `make gw-sync-model-registry`. |
