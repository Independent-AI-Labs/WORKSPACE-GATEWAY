...
| ID | Requirement |
|----|-------------|
| FR-5.1 | The dashboard MUST expose a `rejection_mode` toggle (Grafana custom variable) with values `binary` and `instances`; the toggle changes only query-side aggregation: stored data is always granular. |
...
| ID | Requirement |
|----|-------------|
| FR-6.1 | Two provisioned dashboards: `gateway-model-experience` (uid `gateway-model-experience`, satisfaction/friction panels: ids 32-35, 37, 38, 40-43) and `gateway-model-performance` (uid `gateway-model-performance`, speed/reliability/waste panels: ids 30, 31, 36, 39, 44): MUST follow REQ-DASHBOARD structural rules (FR-6.x there) and define the same `api_key` + `model` multi-value template variables; `rejection_mode` lives only on the experience dashboard. |
...
| FR-9.5 | Interpretation bands MUST be shown: ≥ 70 good (green), 40-69 mixed (yellow), < 40 poor (red); a per-construct scorecard (raw rates, standardized indices, PAI, Reliability, Overall, friction as context) MUST accompany the score so the composite is never a black box. Both panels carry the behavioral-heuristic caveat and the session-abandonment contamination warning (multi-model interleaving inflates abandonment for models used in mixed workflows). |
...
| ID | Requirement |
|----|-------------|
| FR-10.1 | New panels on `gateway-model-experience`: (a) **Usefulness Score leaderboard**: one bargauge row per qualifying model, verdict-colored per FR-9.5; (b) **scorecard**: table of per-model normalized factors feeding the score; (c) **friction rate** stacked timeseries with the three FR-8.5 classes; (d) **top guard rules** ranked bar (like top-profanities). |
| FR-10.2 | Readability rules for existing panels: rate gauges carry green/amber/red threshold bands; speed panels show p50 alongside mean; every ratio panel states its denominator in the description; sparse buckets stay absent per FR-5.5; the historical tok/s proxy keeps its ESTIMATE label until native TTFT rows dominate. |
| FR-10.3 | Human-readable presentation: every table column and stat series carries a human name + unit (quoted SQL aliases / `displayName` overrides: no `A cancel_pct`-style identifiers); the top-terms panel becomes a single merged **Top User Rejection Strings** table (Category column, no A/B tabs) with profanity terms censored to first/last character (`b***h`); the rejection stat title states metric and unit ("User Rejection Rate (% of follow-up messages)"). |
| FR-10.4 | Waste is quantified in absolute terms, proportionally and monetarily: wasted tokens, wasted as % of all completion tokens, and $ cost of wasted tokens as an exact `"$x.yy"` string (REQ-DASHBOARD FR-7.1), next to cost per completed response. |
| FR-10.5 | Panels are split across two dashboards, each grouped in tiers. `gateway-model-experience`: Verdict (score + scorecard) → headline stats (rejection, baseline-net, session depth) → rejection over time + top strings → friction + guard rules + model switch. `gateway-model-performance`: prefill p30 / decode p44 side by side (separate panels so the ~40k prefill and ~100 decode scales never share an axis) → cancel/abort + waste + historical proxy. Row-keyed bargauges use all-values reduce (one gauge per model). |
...
| NFR-1.2 | A full re-crunch of the retained history (13 months) MUST be resumable window-by-window and MUST NOT lock or block dashboard queries beyond ClickHouse's normal merge behavior. |
...
| Q10 | Score-family revision of the original weighted-linear FR-9 (which conflated satisfaction/friction/speed, normalized against observed extrema, and allowed compensatory trade-offs). **Resolved 2026-09-16: implemented per FR-9 (revised):** Overall = sqrt(PAI × Reliability) with fixed goalposts and geometric aggregation; PAI = prompt-adherence indices (net rejection 0-30%, session abandonment 0-100%, client cancel 0-5%); friction and speed excluded as separate constructs (friction context columns + dedicated panels; speed on the performance dashboard). Research grounding: SUM (Sauro & Kindlund 2005: z-vs-specification-limit standardization, equal construct weights via PCA), HDI geometric mean (UNDP 2010: no substitutability across dimensions; Anand critique: goalpost sensitivity), Nielsen 0.1/1/10s response-time limits, Tan et al. 2026 (arXiv 2604.06183: 2s responses rated *less* thoughtful than 9-20s: speed-quality non-monotone, so speed informs routing, not the score), SPUR (ACL 2024: interpretable satisfaction rubrics). |
...
| FR-5.x weighting/normalization | Implemented | request_signals.signal_weight; gateway-model-experience.json (rejection_mode variable, per-bucket denominators, sparse-bucket HAVING, signed net, parsed-gated baseline) |
| FR-6.x dashboard | Implemented | conf/grafana/dashboards/gateway-model-experience.json (10 panels) + gateway-model-performance.json (5 panels); tests/config/test_dashboard_experience.sh + test_dashboard_performance.sh; REQ-DASHBOARD FR-1.1 amended to 5 dashboards |
...
| FR-9.x score family | Implemented | gateway-model-experience.json p40/p41 (Overall = sqrt(PAI × Reliability), fixed goalposts 30/100/5/5, geometric aggregation, ≥30-request gate, friction/speed excluded); live: 12 models ranked, kimi-k3 88.7 / glm-5.3 81.9 / glm-5.2 0.0 |
| FR-10.x dashboard refinement | Implemented | p40-p43 + p50 companions on p30; FR-10.3/4/5 readability pass (censored merged strings table, %/USD waste, human names, tiered layout split across experience/performance dashboards); test_dashboard_experience.sh 62/62 + test_dashboard_performance.sh 44/44 |
