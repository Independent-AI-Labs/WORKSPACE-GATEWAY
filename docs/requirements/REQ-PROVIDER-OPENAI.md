...
- `provider-oauth` device/browser start, callback, bearer validation, refresh, and header injection
...
| FR-1.2 | The route MUST use `provider-oauth`; unauthenticated requests MUST NOT reach the upstream. |
...
| FR-1.8 | `workspace-gw-openai-device-oauth` MUST use route `/openai`, OAuth plugin `provider-oauth`, npm package `@ai-sdk/openai`, and models.dev namespace `openai`. |
...
`provider-oauth`, which performs the token exchange and returns a gateway-issued
client credential.
...
| OAuth plugin | Implemented | `plugins/custom/provider-oauth.lua` (generic plugin; OpenAI is a `chatgpt_device` config set on `relay-openai`) |
...
| Browser PKCE OAuth parity | Implemented | `provider-oauth.lua` (`browser_flow`), `oauth_device.lua`, and gateway-owned OpenCode plugin |
...
| OpenCode request-header parity | Implemented | `provider-oauth.lua` injects `originator` and forwards `session-id`; dynamic platform User-Agent remains pinned |
