...
| FR-2.5 | `relay-kimi` (`/kimi/*`) and `relay-kimi-v1` (`/kimi/v1/*`) MUST rewrite to `/coding/v1/$1` on `api.kimi.com:443` and MUST attach `provider-oauth` (federated OAuth device-flow auth). |
...
| ID | Requirement |
|----|-------------|
| FR-4.1 | [`conf/config.yaml`](../../conf/config.yaml) MUST register custom plugins `key-resolver`, `key-meta`, `provider-oauth`, `provider-sync`, `sse-usage`, `redact` in the `plugins:` list. |
...
|----|-----------|--------|
| C-1 | APISIX 3.18.0 (`apache/apisix:3.18.0-debian`) | RUNBOOK-DEPLOYMENT |
