...
if [ -z "$PODMAN_BIN" ]; then
    if command -v real-podman 2>&1; then
...
# ---------------------------------------------------------- dry-run
DRY_OUT=$(OPENCODE_DBS="$SRC_COPY" bash "$MIGRATOR" --dry-run --clickhouse-url "$CH_URL")
echo "$DRY_OUT"
assert_eq "dry-run rows to insert (usage)" \
    "[DRY-RUN] rows to insert: usage_log=2 request_log=2" \
...
RUN1=$(OPENCODE_DBS="$SRC_COPY" bash "$MIGRATOR" --clickhouse-url "$CH_URL" --backup-dir "$BACKUP_DIR")
echo "$RUN1"
assert_eq "run1 usage_log inserted" \
    "[OK] usage_log: inserted=2 skipped_existing=0 of 2" \
    "$(echo "$RUN1" | grep 'usage_log:')"
assert_eq "run1 request_log inserted" \
    "[OK] request_log: inserted=2 skipped_existing=0 of 2" \
...
assert_eq "cost_source unknown (cost=0)" "unknown" "$(echo "$U2" | jq -r .cost_source)"
...
# ---------------------------------------------------------- idempotency
RUN2=$(OPENCODE_DBS="$SRC_COPY" bash "$MIGRATOR" --clickhouse-url "$CH_URL")
echo "$RUN2"
assert_eq "run2 usage_log idempotent" \
    "[OK] usage_log: inserted=0 skipped_existing=2 of 2" \
    "$(echo "$RUN2" | grep 'usage_log:')"
assert_eq "run2 request_log idempotent" \
    "[OK] request_log: inserted=0 skipped_existing=2 of 2" \
    "$(echo "$RUN2" | grep 'request_log:')"
assert_eq "usage_log total unchanged" "2" \
    "$(chq 'SELECT count() FROM llm_gateway.usage_log')"
assert_eq "request_log total unchanged" "2" \
...
if [ "$FULL" = true ]; then
    if [ ! -f "$OPENCODE_LIVE_DB" ]; then
        ...
        DRY2=$(OPENCODE_DBS="$LIVE_COPY" bash "$MIGRATOR" --dry-run --clickhouse-url "$CH2_URL")
        echo "$DRY2"
        EXPECT=$(echo "$DRY2" | grep 'rows to insert' | sed -E 's/.*usage_log=([0-9]+).*/\1/')

        RUN3=$(OPENCODE_DBS="$LIVE_COPY" bash "$MIGRATOR" --clickhouse-url "$CH2_URL" --backup-dir "$TMPD/live-backup")
        echo "$RUN3"
        GOT=$(echo "$RUN3" | grep 'usage_log:' | sed -E 's/.*inserted=([0-9]+).*/\1/')
