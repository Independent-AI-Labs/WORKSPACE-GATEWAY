...
# Panel: p20 Top Clients by Cost & Tokens (stat panel, 10 ranked tiles like p3)
...
# Panel count and datasource split (1 panel, ClickHouse)
assert_eq "$LABEL: panel count is 1" "1" "$(jq '.panels|length' "$F")"
assert_eq "$LABEL: ClickHouse panels" "1" "$(jq '[.panels[]|select(.datasource.uid=="clickhouse")]|length' "$F")"
