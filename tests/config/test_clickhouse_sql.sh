...
HAS_DB_RC=0
HAS_DB=$(grep -c 'CREATE DATABASE.*llm_gateway' "$SQL_FILE" ) || { fail_RC=$?; fail=""; }
assert_eq "Creates database llm_gateway" "1" "$HAS_DB"

HAS_REQUEST_LOG_RC=0
HAS_REQUEST_LOG=$(grep -c 'CREATE TABLE.*request_log' "$SQL_FILE" ) || { HAS_DB_RC=$?; HAS_DB=""; }
assert_eq "Creates table request_log" "1" "$HAS_REQUEST_LOG"

HAS_BILLING_LEDGER_RC=0
HAS_BILLING_LEDGER=$(grep -c 'CREATE TABLE IF NOT EXISTS llm_gateway.billing_ledger' "$SQL_FILE" ) || { HAS_REQUEST_LOG_RC=$?; HAS_REQUEST_LOG=""; }
assert_eq "Creates table billing_ledger" "1" "$HAS_BILLING_LEDGER"

HAS_BILLING_DISC_RC=0
HAS_BILLING_DISC=$(grep -c 'CREATE TABLE IF NOT EXISTS llm_gateway.billing_discrepancies' "$SQL_FILE" ) || { HAS_BILLING_LEDGER_RC=$?; HAS_BILLING_LEDGER=""; }
assert_eq "Creates table billing_discrepancies" "1" "$HAS_BILLING_DISC"

HAS_DECIMAL_RC=0
HAS_DECIMAL=$(grep -c 'cost.*Decimal64(6)' "$SQL_FILE" ) || { HAS_BILLING_DISC_RC=$?; HAS_BILLING_DISC=""; }
assert_eq "billing_ledger has Decimal64(6) for cost" "true" "$(if [ "$HAS_DECIMAL" -ge 1 ]; then printf 'true'; else printf 'false'; fi)"

HAS_TTL_RC=0
HAS_TTL=$(grep -c 'INTERVAL 13 MONTH' "$SQL_FILE" ) || { HAS_DECIMAL_RC=$?; HAS_DECIMAL=""; }
assert_eq "TTL 13 MONTH on tables" "true" "$(if [ "$HAS_TTL" -ge 2 ]; then printf 'true'; else printf 'false'; fi)"

HAS_PART_LIMITS_RC=0
HAS_PART_LIMITS=$(grep -c '^[[:space:]]*parts_to_throw_insert = 1000' "$SQL_FILE" ) || { HAS_TTL_RC=$?; HAS_TTL=""; }
...
HAS_INACTIVE_PART_LIMITS=$(grep -c 'inactive_parts_to_throw_insert = 1000' "$SQL_FILE" ) || { HAS_PART_LIMITS_RC=$?; HAS_PART_LIMITS=""; }
...
HAS_RUNTIME_PART_LIMITS=$(grep -c '^ALTER TABLE.*MODIFY SETTING' "$SQL_FILE" ) || { HAS_INACTIVE_PART_LIMITS_RC=$?; HAS_INACTIVE_PART_LIMITS=""; }
assert_eq "Existing MergeTree tables receive runtime part limits" "4" "$HAS_RUNTIME_PART_LIMITS"

HAS_TOTAL_PART_LIMITS_RC=0
HAS_TOTAL_PART_LIMITS=$(grep -c 'max_parts_in_total = 5000' "$SQL_FILE" ) || { HAS_RUNTIME_PART_LIMITS_RC=$?; HAS_RUNTIME_PART_LIMITS=""; }
assert_eq "Created and existing MergeTree tables cap total parts" "8" "$HAS_TOTAL_PART_LIMITS"

ORDER_BY_LEADING_RC=0
ORDER_BY_LEADING=$(grep -o 'ORDER BY ([a-z_]*' "$SQL_FILE" ) || { HAS_TOTAL_PART_LIMITS_RC=$?; HAS_TOTAL_PART_LIMITS=""; }
LEADING_COUNT_RC=0
LEADING_COUNT=$(echo "$ORDER_BY_LEADING" | grep -c 'ORDER BY (provider\|ORDER BY (tenant_id\|ORDER BY (date' ) || { ORDER_BY_LEADING_RC=$?; ORDER_BY_LEADING=""; }
assert_eq "ORDER BY leads with low-cardinality keys" "3" "$LEADING_COUNT"

HAS_PROMPT_TOKENS_RC=0
HAS_PROMPT_TOKENS=$(grep -c 'prompt_tokens' "$SQL_FILE" ) || { LEADING_COUNT_RC=$?; LEADING_COUNT=""; }
...
HAS_COMPLETION_TOKENS=$(grep -c 'completion_tokens' "$SQL_FILE" ) || { HAS_PROMPT_TOKENS_RC=$?; HAS_PROMPT_TOKENS=""; }
...
HAS_ABORTED_ALTER=$(grep -c 'ADD COLUMN IF NOT EXISTS aborted' "$SQL_FILE" ) || { HAS_ABORTED_RC=$?; HAS_ABORTED=""; }
assert_eq "Has idempotent ALTER for aborted column" "1" "$HAS_ABORTED_ALTER"

HAS_IS_STREAM_RC=0
HAS_IS_STREAM=$(grep -c 'is_stream.*UInt8' "$SQL_FILE" ) || { HAS_ABORTED_ALTER_RC=$?; HAS_ABORTED_ALTER=""; }
...
HAS_IS_STREAM_ALTER=$(grep -c 'ADD COLUMN IF NOT EXISTS is_stream' "$SQL_FILE" ) || { HAS_IS_STREAM_RC=$?; HAS_IS_STREAM=""; }
assert_eq "Has idempotent ALTER for is_stream column" "1" "$HAS_IS_STREAM_ALTER"

HAS_COST_RC=0
HAS_COST=$(grep -c 'cost.*Float64' "$SQL_FILE" ) || { HAS_IS_STREAM_ALTER_RC=$?; HAS_IS_STREAM_ALTER=""; }
assert_eq "Has cost Float64 column in usage_log" "true" "$(if [ "$HAS_COST" -ge 1 ]; then printf 'true'; else printf 'false'; fi)"

HAS_COST_ALTER_RC=0
HAS_COST_ALTER=$(grep -c 'ADD COLUMN IF NOT EXISTS cost ' "$SQL_FILE" ) || { HAS_COST_RC=$?; HAS_COST=""; }
assert_eq "Has idempotent ALTER for cost column" "1" "$HAS_COST_ALTER"

HAS_COST_SOURCE_RC=0
HAS_COST_SOURCE=$(grep -c "cost_source.*Enum8" "$SQL_FILE" ) || { HAS_COST_ALTER_RC=$?; HAS_COST_ALTER=""; }
...
HAS_COST_SOURCE_ALTER=$(grep -c 'ADD COLUMN IF NOT EXISTS cost_source' "$SQL_FILE" ) || { HAS_COST_SOURCE_RC=$?; HAS_COST_SOURCE=""; }
assert_eq "Has idempotent ALTER for cost_source column" "1" "$HAS_COST_SOURCE_ALTER"

HAS_ENUM_VALUES_RC=0
HAS_ENUM_VALUES=$(grep -c "Enum8('upstream' = 0, 'computed' = 1, 'unknown' = 2)" "$SQL_FILE" ) || { HAS_COST_SOURCE_ALTER_RC=$?; HAS_COST_SOURCE_ALTER=""; }
...
HAS_BILLING_MV_RC=0
HAS_BILLING_MV=$(grep -c 'CREATE MATERIALIZED VIEW IF NOT EXISTS.*billing_ledger_mv' "$SQL_FILE" ) || { HAS_ENUM_VALUES_RC=$?; HAS_ENUM_VALUES=""; }
assert_eq "Creates materialized view billing_ledger_mv" "1" "$HAS_BILLING_MV"

HAS_MV_TO_RC=0
HAS_MV_TO=$(grep -c 'TO llm_gateway.billing_ledger' "$SQL_FILE" ) || { HAS_BILLING_MV_RC=$?; HAS_BILLING_MV=""; }
assert_eq "MV targets billing_ledger table" "1" "$HAS_MV_TO"

HAS_MV_FROM_USAGE_RC=0
HAS_MV_FROM_USAGE=$(grep -c 'FROM llm_gateway.usage_log' "$SQL_FILE" ) || { HAS_MV_TO_RC=$?; HAS_MV_TO=""; }
assert_eq "MV selects FROM usage_log" "1" "$HAS_MV_FROM_USAGE"

HAS_MV_REQUEST_MODE_RC=0
HAS_MV_REQUEST_MODE=$(grep -c "request_mode" "$SQL_FILE" ) || { HAS_MV_FROM_USAGE_RC=$?; HAS_MV_FROM_USAGE=""; }
...
HAS_MV_CACHE_STATUS=$(grep -c "cache_status" "$SQL_FILE" ) || { HAS_MV_REQUEST_MODE_RC=$?; HAS_MV_REQUEST_MODE=""; }
assert_eq "MV has cache_status column" "true" "$(if [ "$HAS_MV_CACHE_STATUS" -ge 1 ]; then printf 'true'; else printf 'false'; fi)"

HAS_MV_SUCCESS_RC=0
HAS_MV_SUCCESS=$(grep -c "aborted = 0" "$SQL_FILE" ) || { HAS_MV_CACHE_STATUS_RC=$?; HAS_MV_CACHE_STATUS=""; }
...
for table in metric_log asynchronous_metric_log text_log trace_log processors_profile_log; do
    HAS_REMOVE_RC=0
    HAS_REMOVE=$(grep -c "<${table} remove=\"1\"" "$CH_LOG_XML" ) || { HAS_MV_SUCCESS_RC=$?; HAS_MV_SUCCESS=""; }
    assert_eq "ClickHouse ${table} flush disabled" "1" "$HAS_REMOVE"

    HAS_TRUNCATE=$(grep -c "TRUNCATE TABLE IF EXISTS system.${table};" "$SQL_FILE" ) || { HAS_REMOVE_RC=$?; HAS_REMOVE=""; }
