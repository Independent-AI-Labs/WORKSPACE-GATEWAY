...
HAS_DB=$(grep -c 'CREATE DATABASE.*llm_gateway' "$SQL_FILE" || echo "")
assert_eq "Creates database llm_gateway" "1" "$HAS_DB"

HAS_REQUEST_LOG=$(grep -c 'CREATE TABLE.*request_log' "$SQL_FILE" || echo "")
assert_eq "Creates table request_log" "1" "$HAS_REQUEST_LOG"

HAS_BILLING_LEDGER=$(grep -c 'CREATE TABLE IF NOT EXISTS llm_gateway.billing_ledger' "$SQL_FILE" || echo "")
assert_eq "Creates table billing_ledger" "1" "$HAS_BILLING_LEDGER"

HAS_BILLING_DISC=$(grep -c 'CREATE TABLE IF NOT EXISTS llm_gateway.billing_discrepancies' "$SQL_FILE" || echo "")
assert_eq "Creates table billing_discrepancies" "1" "$HAS_BILLING_DISC"

HAS_DECIMAL=$(grep -c 'cost.*Decimal64(6)' "$SQL_FILE" || echo "")
