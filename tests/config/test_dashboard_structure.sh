...
# v44: p1 Total Requests and p7 Status Code Breakdown moved from Prometheus to ClickHouse
...
assert_eq "Panels using Prometheus datasource" "5" "$PROM_PANELS"

CH_PANELS=$(jq '[.panels[] | select(.datasource.uid == "clickhouse")] | length' "$DASHBOARD_FILE")
assert_eq "Panels using ClickHouse datasource" "9" "$CH_PANELS"
...
# S15: p4 Error Rate query uses countIf(status >= 400) and $__timeFilter
# v44: Error Rate now counts ALL 4xx+5xx errors (status >= 400), not just 5xx
P4_SQL=$(jq -r '[.panels[] | select(.id == 4)][0].targets[0].rawSql' "$DASHBOARD_FILE")
echo "$P4_SQL" | grep -q 'countIf(status >= 400)' && { echo "[PASS] S15: p4 uses countIf(status >= 400)"; pass=$((pass+1)); } || { echo "[FAIL] S15: p4 missing countIf(status >= 400)"; fail=$((fail+1)); }
...
# S19: Removed - prometheus.yaml was in tests/integration/queries/ which was deleted
# (queries are now extracted from the dashboard JSON directly, no duplicated .sql/.yaml files)

# S20: Dashboard version is 44
DASH_VERSION=$(jq -r '.version' "$DASHBOARD_FILE")
assert_eq "S20: Dashboard version is 44" "44" "$DASH_VERSION"
