...
DIRECT_KEY="${OPENCODE_ZEN_API_KEY:-sk-test-direct}"
...
test_no_auth() {
    ...
    if [ "$code" = "401" ]; then
        record_pass "key-resolver rejects without Authorization header (401)"
        return 0
    fi
    record_fail "key-resolver without Authorization returned $code, expected 401"
    return 1
}
...
test_wrong_key() {
    ...
    if [ "$code" = "401" ]; then
        record_pass "key-resolver rejects wrong vgw- key (401)"
        return 0
    fi
    record_fail "key-resolver with wrong vgw- key returned $code, expected 401"
    return 1
}

test_direct_key_passthrough() {
    local code
    code=$(http_code -H "Authorization: Bearer $DIRECT_KEY" "$GATEWAY$ROUTE")
    if [ "$code" = "401" ]; then
        record_fail "direct key (non-vgw) was rejected by gateway, expected pass-through to upstream"
        return 1
    fi
    record_pass "key-resolver passes through direct key (non-vgw, got $code)"
    return 0
}
...
test_correct_key_not_401() {
    ...
    record_pass "key-resolver accepts correct key (non-401: got $code)"
    return 0
}

test_issued_key_works() {
    local new_key
    ...
    local code
    code=$(http_code -H "Authorization: Bearer $new_key" "$GATEWAY$ROUTE")
    if [ "$code" = "401" ]; then
        record_fail "issued key returned 401, expected non-401"
        return 1
    fi
    record_pass "key-resolver accepts newly issued key (non-401: got $code)"
    return 0
}
...
test_revoked_key_rejected() {
    ...
    local code_before
    code_before=$(http_code -H "Authorization: Bearer $rev_key" "$GATEWAY$ROUTE")
    if [ "$code_before" = "401" ]; then
        record_fail "key should work before revocation"
        return 1
    fi
    ...
    local code_after
    code_after=$(http_code -H "Authorization: Bearer $rev_key" "$GATEWAY$ROUTE")
    if [ "$code_after" = "401" ]; then
        record_pass "key-resolver rejects revoked key (401)"
        return 0
    fi
    record_fail "revoked key returned $code_after, expected 401"
    return 1
}
