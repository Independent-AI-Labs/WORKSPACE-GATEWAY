...
SELECT concat(multiIf(cached_tok >= 1000000, concat(toString(round(cached_tok / 1000000)), ' Mil'), cached_tok >= 1000, concat(toString(round(cached_tok / 1000)), ' K'), toString(cached_tok)), ' ($', toString(round(total_cost * cached_tok / nullIf(total_tok, 0), 2)), ')') as val FROM totals
