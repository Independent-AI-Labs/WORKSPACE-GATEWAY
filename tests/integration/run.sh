...
if stack_is_up; then
    for test_script in test_key_resolver.sh test_route_relay.sh test_prometheus.sh test_grafana.sh test_dashboard_queries.sh test_grafana_ds_proxy.sh test_grafana_panels.sh test_provider_sync_client.sh test_recalc_costs_run.sh; do
        echo ""
        echo "--- $test_script ---"
